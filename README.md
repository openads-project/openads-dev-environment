...
| `root_ci_workflows_match_templates` | Passes when the root workflow files `.github/workflows/docs.yml` and `.github/workflows/consistency.yml` match the corresponding templates in `.openads-dev-environment/.github/workflow_calls/`, while allowing template placeholders such as `{{ skipped_check_ids }}` to be replaced with repository-specific values. |
