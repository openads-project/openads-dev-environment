...
6. *(optional)* [Check consistency]((#consistency-checker) of your repository with OpenADS conventions.
   ```bash
...
| `required_top_level_symlinks` | Passes when the repository root contains symlinks `.devcontainer -> .openads-dev-environment/.devcontainer/` and `.vscode -> .openads-dev-environment/.vscode/`. |
